...
- 内容地图 / 概念闭环类关系图优先使用 `template: relationship` + `metadata.layout_pattern: "content-map-orbit"`：主节点按业务闭环顺时针放在外环，中心只放主题岛，治理、可信度、验收点等 guardrail 放底栏；不要把 8 个以上文本框挤在中心区，也不要让反馈线穿过主题岛或正文。
